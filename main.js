...
async function searchByCountry() {
    ...
    try {
        ...
        const geography = `${countryData.name} is located in ${countryData.subregion}.`
        //console.log(geography);

        const population = `This gorgeous country has a population of ${countryData.population} human beings.`;
        //console.log(population);

        const countryCapital = `Its capital is ${countryData.capital} `;
        //console.log(countryCapital);

        const currency = `and you can pay there with ${countryData.currencies[0].name}'s.`;
        //console.log(currency);

        const spokenLanguage = `Their native language is ${countryData.languages[0].name}.`
